...
serve(async (req) => {
  ...
  try {
    ...
    // IMPORTANT: If you haven't verified a custom domain in Resend, 
    // you MUST use 'onboarding@resend.dev' as the sender adress.
    ...
    const emailResponse = await fetch(RESEND_API_URL, {
      ...
      body: JSON.stringify({
        from: `Bárbaro Store <${senderEmail}>`,
        to: [recipientEmail],
        subject: `Notificación: ${title}`,
        html: `
          <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #eee; border-radius: 10px; overflow: hidden;">
             <div style="background-color: #000; color: #fff; padding: 20px; text-align: center;">
                <h1 style="margin: 0; font-size: 24px;">Bárbaro Store</h1>
             </div>
             <div style="padding: 30px; line-height: 1.6; color: #333;">
                <h2 style="margin-top: 0;">Hola ${recipientName},</h2>
                <p>${message}</p>
                ${link_url ? `<div style="margin-top: 30px; text-align: center;"><a href="${link_url}" style="background-color: #2b8cee; color: white; padding: 12px 25px; text-decoration: none; border-radius: 5px; font-weight: bold; display: inline-block;">Ver Detalles en la App</a></div>` : ''}
             </div>
             <div style="background-color: #f9f9f9; padding: 20px; text-align: center; font-size: 12px; color: #888; border-top: 1px solid #eee;">
                <p style="margin: 0;">Este es un correo automático de tu sistema de gestión Barbaro.</p>
                <p style="margin: 5px 0 0;">Si tienes dudas contacta a soporte.</p>
             </div>
          </div>
        `
      })
